...
def main():
    ...
    if not(model_name=='ss') and not(model_name=='cosso'): 
        model_name = 'lgbayescos'
    
    if model_name=='lgbayescos':
        output_prefix = os.path.abspath(
    ...
    for train_index, test_index in kf.split(X):
        try :
            ...
            if adaptive == 'Adaptive': 
                ...
                if model_name == 'lgbayescos' and isotropic_info and mixing=='Dirichlet':
            ...
            out = estimator.train(X_train, y_train, **kwargs)
            print("ended")
            ...
            if not(model_name=='ss'):
                # For Bayesian, this returns a set of sample fors each selection parameter
                main_selection, interaction_selection = estimator.selection_parameter
                main_selection = main_selection.T
                interaction_selection = interaction_selection.T
            ...
            if model_name=='lgbayescos':
                estimator.predict_on(num_samples=num_samples_inference, subsample=subsample) 
            ...
            if model_name=='lgbayescos':
                    estimator.predict_on(num_samples=num_samples_inference//10, subsample=subsample) 
            ...
            y_pred_mean = estimator.constant_mean.detach().numpy()
            if model_name == 'lgbayescos':
            ...
            main_labels = [i[:6]+'.' if len(i)>4 else i[:6] for i in covariate_names[dataset_name]] # les 4 premiers lettres du nom
            inter_labels = ['['+main_labels[i]+','+main_labels[j]+']' for i in np.arange(0, D - 1)
                            for j in np.arange(i + 1, D)]
            
            main_labels = [str(i+1) for i in range(len(covariate_names[dataset_name]))] # les 4 premiers lettres du nom
            inter_labels = ['['+main_labels[i]+','+main_labels[j]+']' for i in np.arange(0, D - 1)
                            for j in np.arange(i + 1, D)]
            ...
            # Plot selection parameter (10 first in each order) for cosso and lgbayescos
            if not(model_name=='ss'): 
                main_order = order[order<D]
                main_best_components_labels = [main_labels[i] for i in main_order[:10]]
                main_best_selection = [main_selection[i] for i in main_order[:10]]
                
                plt.figure(figsize=(9, 5))
                
                if model_name == 'cosso':
                    fig, ax = plt.subplots()
                    ax.plot(np.arange(1, 1+len(main_best_selection)), main_best_selection, "b", linewidth=3)
                    ax.plot(np.arange(1, 1+len(main_best_selection)), main_best_selection, '*b', linewidth=3)
                    plt.xticks(np.arange(1, 1+len(main_best_selection)), main_best_components_labels, rotation=90, fontsize=16)
                else:
                    fig, ax = plt.subplots()
                    ax.boxplot(main_best_selection, labels= main_best_components_labels)
                    ax.plot(np.arange(1, 1+len(main_best_selection)), np.median(main_best_selection, axis=-1), "b", linewidth=3)
                    plt.xticks(np.arange(1, 1+len(main_best_selection)), main_best_components_labels, rotation=90, fontsize=16)
                
                plt.grid()
                plt.xlabel('Active main components')
                plt.ylabel('Selection parameters')
                plt.title(dataset_name)
                plt.tight_layout()
                plt.savefig(output_prefix + "/Active_main_components_selection_%d.png" % fold)
                
                
                interaction_order = order[order>=D]-D
                interaction_best_components_labels = [inter_labels[i] for i in interaction_order[:10]]
                interaction_best_selection = [interaction_selection[i] for i in interaction_order[:10]]
                
                plt.figure(figsize=(9, 5))
                if model_name == 'cosso':
                    fig, ax = plt.subplots()
                    ax.plot(np.arange(1, 1+len(interaction_best_selection)), interaction_best_selection, "b", linewidth=3)
                    ax.plot(np.arange(1, 1+len(interaction_best_selection)), interaction_best_selection, '*b', linewidth=3)
                    plt.xticks(np.arange(1, 1+len(interaction_best_selection)), interaction_best_components_labels, rotation=90, fontsize=16)
                else:
                    fig, ax = plt.subplots()
                    ax.boxplot(interaction_best_selection, labels=interaction_best_components_labels)
                    ax.plot(np.arange(1, 1+len(interaction_best_selection)), np.median(interaction_best_selection, axis=-1), "b", linewidth=3)
                    plt.xticks(np.arange(1, 1+len(interaction_best_selection)), interaction_best_components_labels, rotation=90, fontsize=16)
                
                plt.grid()
                plt.xlabel('Active interaction components')
                plt.ylabel('Selection parameters')
                plt.title(dataset_name)
                plt.tight_layout()
                plt.savefig(output_prefix + "/Active_interaction_components_selection_%d.png" % fold)
            ...
            np.savez(
                ...
                coverage1=coverage1,
                coverage2=coverage2,
                components_variance=components_variance,
                order=order,
                rmse=rmse,
                mae=mae,
                r2=r2)

...
def main():
    ...
    for id, filename in enumerate(filenames[:10]): 
        ...
        if model_name=='ss':
                ...
                mean_prior = gpytorch.priors.NormalPrior(0, 10.0) 
        ...
        f_pred = estimator.predict(X_test, kind='latent')
        ...
        if model_name=='ss':
                ...
                np.savez(output_prefix + "/alpha_star_%d" % id, alpha_star=alpha_star)
                
                _log.info(f"Best results for alpha={alpha_star} giving error={ss_loss.min()}")
                
        elif model_name=='cosso':
                M_star, cosso_loss = out
                np.savez(output_prefix + "/M_star_%d" % id, M_star=M_star)
